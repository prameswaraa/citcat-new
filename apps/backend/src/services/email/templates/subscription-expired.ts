/**
 * Subscription Expired Email Template
 * 
 * Generates HTML and plain text email content for subscription expiry notification
 * Requirements: 11.4 - Notify user when subscription expires and account is downgraded
 */

import { escapeHtml } from '../../../utils/sanitize.js';

export interface SubscriptionExpiredParams {
  /** User's name */
  userName: string;
  /** Previous subscription tier (LITE or PRO) */
  tierName: string;
  /** Date when subscription expired */
  expiredDate: Date;
  /** Application name (defaults to Citcat) */
  appName?: string;
}

export interface SubscriptionExpiredTemplate {
  subject: string;
  html: string;
  text: string;
}

/**
 * Format date to Indonesian locale string
 */
function formatDate(date: Date): string {
  return date.toLocaleDateString('id-ID', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
}

/**
 * Generate subscription expired email template
 * All user-provided content is sanitized before inclusion
 */
export function subscriptionExpiredTemplate(
  params: SubscriptionExpiredParams
): SubscriptionExpiredTemplate {
  const {
    userName,
    tierName,
    expiredDate,
    appName = 'Citcat',
  } = params;

  // Sanitize inputs
  const sanitizedUserName = escapeHtml(userName);
  const sanitizedTierName = escapeHtml(tierName);
  const sanitizedAppName = escapeHtml(appName);
  const formattedExpiredDate = formatDate(expiredDate);

  const subject = `Subscription ${sanitizedTierName} Telah Berakhir - ${sanitizedAppName}`;

  const html = `
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Subscription Berakhir</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    <tr>
      <td align="center" style="padding: 40px 0;">
        <table role="presentation" style="width: 100%; max-width: 520px; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08);">
          <!-- Header -->
          <tr>
            <td style="padding: 32px 32px 24px; text-align: center; border-bottom: 1px solid #eee;">
              <h1 style="margin: 0; font-size: 24px; font-weight: 600; color: #1a1a1a;">${sanitizedAppName}</h1>
            </td>
          </tr>
          
          <!-- Icon -->
          <tr>
            <td style="padding: 32px 32px 16px; text-align: center;">
              <div style="width: 64px; height: 64px; margin: 0 auto; background-color: #6b7280; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                <span style="font-size: 24px; color: #ffffff; font-weight: bold;">i</span>
              </div>
            </td>
          </tr>
          
          <!-- Content -->
          <tr>
            <td style="padding: 0 32px 32px;">
              <h2 style="margin: 0 0 16px; font-size: 22px; font-weight: 600; color: #1a1a1a; text-align: center;">
                Subscription Telah Berakhir
              </h2>
              <p style="margin: 0 0 24px; font-size: 15px; line-height: 1.6; color: #666; text-align: center;">
                Halo ${sanitizedUserName}, subscription ${sanitizedTierName} Anda telah berakhir pada ${formattedExpiredDate}.
              </p>
              
              <!-- Status Box -->
              <div style="background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin-bottom: 24px; text-align: center;">
                <p style="margin: 0 0 8px; font-size: 14px; color: #666;">Status Akun Saat Ini:</p>
                <span style="display: inline-block; background-color: #9ca3af; color: #ffffff; padding: 6px 16px; border-radius: 4px; font-size: 14px; font-weight: 600;">
                  FREE
                </span>
              </div>
              
              <!-- Info Box -->
              <div style="background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                <p style="margin: 0 0 12px; font-size: 14px; color: #991b1b; font-weight: 600;">
                  Fitur yang tidak lagi tersedia:
                </p>
                <ul style="margin: 0; padding-left: 20px; font-size: 14px; color: #666; line-height: 1.8;">
                  <li>Unlimited messages</li>
                  <li>Advanced analytics</li>
                  <li>Priority support</li>
                  <li>AI-powered responses</li>
                </ul>
              </div>
              
              <!-- CTA Button -->
              <div style="text-align: center; margin-bottom: 24px;">
                <a href="${process.env.FRONTEND_URL || 'https://app.kirim.chat'}/en/subscription"
                   style="display: inline-block; background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-size: 16px; font-weight: 600;">
                  Aktifkan Kembali
                </a>
              </div>
              
              <p style="margin: 0; font-size: 14px; color: #888; text-align: center; line-height: 1.6;">
                Data Anda tetap aman dan tersimpan. Anda dapat mengaktifkan kembali subscription kapan saja.
              </p>
            </td>
          </tr>
          
          <!-- Footer -->
          <tr>
            <td style="padding: 24px 32px; background-color: #f8f9fa; border-radius: 0 0 12px 12px;">
              <p style="margin: 0; font-size: 12px; color: #999; text-align: center;">
                Butuh bantuan? Hubungi support@kirim.chat
              </p>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>
</html>
`.trim();

  const text = `
${sanitizedAppName} - Subscription Telah Berakhir

Halo ${sanitizedUserName},

Subscription ${sanitizedTierName} Anda telah berakhir pada ${formattedExpiredDate}.

Status Akun Saat Ini: FREE

Fitur yang tidak lagi tersedia:
- Unlimited messages
- Advanced analytics
- Priority support
- AI-powered responses

Data Anda tetap aman dan tersimpan. Anda dapat mengaktifkan kembali subscription kapan saja.

Aktifkan kembali subscription Anda:
${process.env.FRONTEND_URL || 'https://app.kirim.chat'}/en/subscription

Butuh bantuan? Hubungi support@kirim.chat

Ini adalah pesan otomatis. Jangan balas email ini.
`.trim();

  return { subject, html, text };
}
